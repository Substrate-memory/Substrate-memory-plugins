---
description: Check Substrate MCP wiring and authentication before installing or reconnecting.
---
...
2. If the plugin/server is configured but tools are absent, say that it is
   ...
   tell the user to use the host's documented restart or session action. Then
   give this exact prompt for the next session:
