"""Host-neutral Substrate runtime ported from the Hermes plugin.

Same redaction, turn-context, capture-envelope, worker, tool-handler, and
prompt logic as ``plugins/substrate/plugin.py``; only the Hermes ``register``
entry point is replaced by the Claude Code surface in ``mcp_server.py`` and
``hooks/``. Configuration and network access resolve lazily on each call.
"""
...
from substrate_core.client import ClientError, SubstrateClient
...
def flush_capture(timeout: float = 8.0) -> None:
    """Wait briefly for queued capture POSTs (detached worker only)."""
    import time as _time

    try:
        deadline = _time.monotonic() + max(0.0, timeout)
        while _time.monotonic() < deadline:
            if _CAPTURE_WORKER._queue.unfinished_tasks <= 0:  # noqa: SLF001
                return
            _time.sleep(0.05)
    except Exception:
        pass
    ...
    return json.dumps(
        {
            ...
            "message": (
                ...
                "this Claude Code profile a tenant-scoped memory key. The key is "  # HOST PATCH (claude-code): host name in user-facing text only.
...
# ---------------------------------------------------------------------------
# Claude Code entry points (thin aliases over the ported Hermes callbacks)
# ---------------------------------------------------------------------------
...
def get_turn_context(
    ...
    **kwargs: Any,
    ...
    """Bounded pre-turn memory context for the UserPromptSubmit hook."""
    return pre_llm_call(session_id, user_message, conversation_history, **kwargs)
...
def queue_turn_capture(
    ...
    **kwargs: Any,
    ...
    """Nonblocking completed-turn capture for the Stop hook."""
    sync_turn(user_content, assistant_content, session_id=session_id,
              messages=messages or [], **kwargs)


def queue_session_boundary(session_id: Any = "", *, boundary: str = "end",
                           **kwargs: Any) -> None:
    """Session markers for SessionStart(clear)/SessionEnd. Never blocks."""
    end_session(session_id, boundary=boundary, **kwargs)
