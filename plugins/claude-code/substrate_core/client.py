...
def _env_file_token(home: Path) -> str:
    # HOST-HOME PATCH (claude-code): <home>/.env fallback for SUBSTRATE_API_KEY. Hosts here do not
...
def _stored_api_key() -> str:
    ...
    for home in homes:
        for relative in (
            ...
            if value:
                return value
    # HOST-HOME PATCH (claude-code): fall back to SUBSTRATE_API_KEY in the host profile <home>/.env
