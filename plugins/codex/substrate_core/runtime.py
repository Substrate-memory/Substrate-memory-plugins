"""Substrate memory runtime for OpenAI Codex CLI.
...
is generalized (duck-typed) so Codex harnesses can attach it, and
Codex-friendly bridge aliases plus hook entry points are appended at the
end. The Codex MCP server (``server.py``) and hook scripts (``hooks/``)
...
def register(ctx: Any) -> None:
    ...
    runtime works on Hermes and on Codex harnesses (MCP server, hooks).
    """
...
# ---------------------------------------------------------------------------
# Codex bridge aliases and hook entry points (same functions, host names)
# ---------------------------------------------------------------------------

# Pre-turn bounded memory context. Returns the context string, or None when
# memory is unavailable (fail closed). The Codex UserPromptSubmit hook calls
# this before each model request and surfaces the result as additionalContext.
...
def codex_pre_turn_text(
    session_id: str = "",
    user_message: str = "",
    conversation_history: list[Any] | None = None,
    **kwargs: Any,
) -> str | None:
    """Pre-turn memory text for the Codex hook.

    Returns the ``<memory-context>`` block (or the onboarding notice with
    the clickable ``verification_uri_complete`` URL) or None when there is
    nothing to inject. Never raises; never leaks secrets.
    """
    try:
        result = pre_llm_call(
            session_id=session_id,
    ...
    if not isinstance(context, str) or not context:
        return None
    if context.startswith("<substrate-connect>"):
        return context
