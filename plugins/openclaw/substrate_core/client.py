...
def _profile_homes() -> list[Path]:
    """Return the one active OpenClaw home without inspecting other profiles."""
    # OPENCLAW HOST-HOME PATCH (vendored copy): Hermes home resolution replaced
    # with the OpenClaw equivalent in hosthome.py. All limits, hosts,
    # validation, and env var names below are unchanged.
    try:
        from .hosthome import openclaw_home
    except ImportError:  # standalone script layout
        from hosthome import openclaw_home  # type: ignore[no-redef]
    values: list[Path] = [openclaw_home()]
...
def _env_file_token(home: Path) -> str:
    # OPENCLAW HOST-HOME PATCH: <home>/.env fallback for SUBSTRATE_API_KEY. Hosts here do not
...
def _stored_api_key() -> str:
    ...
    for home in homes:
        for relative in (
            ...
            if value:
                return value
    # OPENCLAW HOST-HOME PATCH: fall back to SUBSTRATE_API_KEY in the host profile <home>/.env
