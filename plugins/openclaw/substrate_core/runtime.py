"""Host-neutral Substrate retrieval runtime (OpenClaw port).
...
except ImportError:  # standalone script layout (bridge.py / setup.py)
    import contract  # type: ignore[no-redef]
    import onboarding  # type: ignore[no-redef]
    ...
    return json.dumps(
        {
            ...
            "message": (
                ...
                "this OpenClaw home a tenant-scoped memory key. The key is "
...
# ---------------------------------------------------------------------------
# Single-shot posting for the OpenClaw bridge
# ---------------------------------------------------------------------------


def post_envelope_now(envelope: dict[str, Any], *, timeout: float = 5.0) -> bool:
    """POST one envelope synchronously; used by bridge single-shot processes.

    The long-lived ``_CaptureWorker`` covers in-process use. The Node adapter
    spawns one short-lived ``bridge.py`` process per capture/session event and
    detaches it, so the agent turn is never blocked; this helper performs the
    actual POST inside that detached process. Every failure returns False.
    """
    try:
        SubstrateClient.from_env().post_json(
            "/api/v1/ledger/events",
...
def get_turn_context(
    ...
    **kwargs: Any,
    ...
    """Alias of :func:`pre_llm_call` used by the OpenClaw bridge."""
