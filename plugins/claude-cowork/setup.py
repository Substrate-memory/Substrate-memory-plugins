#!/usr/bin/env python3
"""One-prompt setup and credential migration for the Substrate Cowork plugin.

Connects the active Cowork/Claude home (``~/.claude`` unless ``SUBSTRATE_HOME``
overrides it) using the plugin's RFC 8628 device onboarding: prints the exact
browser approval URL, waits for approval, validates the issued key with an
authenticated capabilities plus search health check, and stores it privately
as ``SUBSTRATE_API_KEY`` in the home's ``.env`` plus the 0600 token file the
client reads first. Standard library only. Never prints a token.
"""
...
from substrate_core.client import tls_context
...
def _validate_token(origin: str, token: str) -> bool:
    try:
        ...
        status, value = _request_with_body(
            origin,
            "/api/v1/memory/search",
            body={"query": "Substrate installation health check", "limit": 1},
            token=token,
        )
...
def _request_with_body(origin: str, path: str, *, body: dict[str, Any],
    ...
    if token:
        headers["Authorization"] = f"Bearer {token}"
    request = urllib.request.Request(origin + path, data=data, headers=headers, method="POST")
    try:
        response = _opener().open(request, timeout=60.0)
    except urllib.error.HTTPError as exc:
        response = exc
    except (urllib.error.URLError, OSError, TimeoutError, socket.timeout) as exc:
        raise SetupError("Could not establish a verified TLS connection to Substrate") from exc
...
def connect(home: Path, origin: str) -> dict[str, Any]:
    """Connect the active home using the plugin's device onboarding."""
    try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Connect the active Cowork home to Substrate")
    parser.add_argument("--home", type=Path)
    parser.add_argument("--hermes-home", dest="hermes_home", type=Path,
                        help=argparse.SUPPRESS)  # alias for Hermes muscle memory
    parser.add_argument("--origin")
    parser.add_argument("--name", help="display name shown for this agent in Substrate")
    args = parser.parse_args(argv)
    active_home = hosthome.active_home().resolve()
    home = (args.home or args.hermes_home or active_home).resolve()
    if home != active_home:
