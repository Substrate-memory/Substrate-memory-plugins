...
def active_home() -> Path:
    # HOST-HOME PATCH (claude-cowork): the active Cowork/Claude home
    # (~/.claude) instead of the Hermes home. See hosthome.py.
    try:
        from . import hosthome
    except ImportError:  # standalone script layout
        import hosthome
    return hosthome.active_home().resolve()
...
# HOST-HOME PATCH (claude-cowork): helper implementing the multi-host spec
...
class OnboardingManager:
    ...
    def _complete(self, value: dict[str, Any]) -> bool:
        ...
        try:
            ...
            # HOST-HOME PATCH (claude-cowork): mirror the key into the 0600
