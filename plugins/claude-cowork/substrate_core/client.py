...
def _profile_homes() -> list[Path]:
    # HOST-HOME PATCH (claude-cowork): resolve the Cowork/Claude config home
    # (~/.claude) instead of the Hermes home. Every other rule in this file
    # (limits, allowed hosts, validation, token-file layout) is unchanged from
    # the Hermes reference so the server side stays compatible.
    try:
        from . import hosthome
    except ImportError:  # standalone script layout
        import hosthome
    return hosthome.profile_homes(__file__)
...
def _env_file_token(home: Path) -> str:
    # HOST-HOME PATCH (claude-cowork): <home>/.env fallback for SUBSTRATE_API_KEY. Hosts here do not
...
def _stored_api_key() -> str:
    ...
    for home in homes:
        for relative in (
            ...
            if value:
                return value
    # HOST-HOME PATCH (claude-cowork): fall back to SUBSTRATE_API_KEY in the host profile <home>/.env
