"""Host-neutral Substrate memory runtime for Claude Cowork (stdlib only).

This module ports the host-neutral logic of the Hermes reference plugin
(``plugins/substrate/plugin.py``): redaction, turn-context request building,
capture envelope building, the nonblocking capture worker, the ``memory_*``
tool handlers, result shaping, error mapping, and ``STATIC_MEMORY_PROMPT``.

Cowork entry points (there is no Hermes ``register(ctx)`` on this host):

- ``get_memory_context(...)`` — UserPromptSubmit hook: bounded
  ``<memory-context>`` injection from ``POST /api/v1/memory/turn-context``.
- ``static_prompt()`` — SessionStart hook and skill content: the frozen static
  memory prompt section.
- ``queue_turn_capture(...)`` — Stop/SubagentStop hooks: nonblocking
  completed-turn capture to ``POST /api/v1/ledger/events``.
- ``queue_session_boundary(...)`` — SessionEnd hook: ``capture_session`` marker.
- ``memory_search`` / ``memory_expand`` / ``memory_evidence`` — MCP tools served
  by ``mcp_server.py`` with schemas identical to the Hermes plugin.

Every failure fails closed: retrieval failure returns empty context and
capture failures never block a turn. Access tokens are never printed or logged.
Configuration and network access are resolved lazily when an entry point runs.
"""
...
def get_memory_context(
    ...
    **kwargs: Any,
    ...
    Cowork entry point wired to the UserPromptSubmit hook: the returned
    ``{"context": block}`` becomes the hook's ``additionalContext``.
    """
    try:
...
def queue_turn_capture(
    ...
    **kwargs: Any,
    ...
    """Cowork entry point for Stop/SubagentStop: snapshot one completed turn.

    Thin alias over :func:`sync_turn`; enqueueing never waits for network I/O.
    """
    sync_turn(
        user_content,
        assistant_content,
...
def queue_session_boundary(
    ...
    **kwargs: Any,
    ...
    """Cowork entry point for SessionEnd: queue a session-complete marker."""
    end_session(
        session_id,
        boundary=boundary,
        platform=platform,
...
def static_prompt() -> str:
    """The frozen static memory prompt section for SessionStart/skill content."""
    return STATIC_MEMORY_PROMPT
    ...
    return json.dumps(
        {
            ...
            "message": (
                ...
                "this Cowork home a tenant-scoped memory key. The key is "
...
# Cowork surface map (replaces Hermes ``register(ctx)``; Cowork has no
# in-process plugin registry, so each surface is wired by a manifest file):
#
# - UserPromptSubmit hook (hooks/hooks.json + hooks/pre_turn.py) -> get_memory_context
# - SessionStart hook (hooks/hooks.json + hooks/session_start.py) -> static_prompt
# - Stop/SubagentStop hooks (hooks/hooks.json + hooks/stop_capture.py) -> queue_turn_capture
# - SessionEnd hook (hooks/hooks.json + hooks/session_end.py) -> queue_session_boundary
# - MCP stdio server (.mcp.json + substrate_core/mcp_server.py) -> memory_search,
#   memory_expand, memory_evidence with the schemas below (identical to Hermes).
# - Static section: frozen per session via the SessionStart hook's
#   additionalContext and skills/substrate-cowork-memory/SKILL.md; dynamic memory stays
#   in the UserPromptSubmit additionalContext path.
COWORK_SURFACE = {
    "hooks": {
        "UserPromptSubmit": "get_memory_context",
        "SessionStart": "static_prompt",
        "Stop": "queue_turn_capture",
        "SubagentStop": "queue_turn_capture",
        "SessionEnd": "queue_session_boundary",
    },
    "mcp_tools": ("memory_search", "memory_expand", "memory_evidence"),
}


def post_turn_capture_now(
    user_content: Any = "",
    assistant_content: Any = "",
    *,
    session_id: str = "",
    ...
    **kwargs: Any,
    ...
    """Build one capture_turn envelope and POST it synchronously (fail-closed).

    Used by the detached Stop-hook runner, which cannot rely on the daemon
    worker because the hook process exits. Returns True when the server
    accepted the envelope; never raises, never logs a token.
    """
    try:
        envelope = _capture_envelope(
            user_content,
...
def post_session_boundary_now(
    ...
    **kwargs: Any,
    ...
    """Build one capture_session envelope and POST it synchronously.

    Used by the detached SessionEnd-hook runner. Idempotent per session via
    the same ``_SENT_SESSIONS`` guard as :func:`end_session`; never raises.
    """
    try:
        sid = _bounded_text(session_id, contract.MAX_SESSION_ID_BYTES)
        if not sid or boundary not in contract.BOUNDARIES:
        ...
        with _SENT_SESSIONS_LOCK:
            ...
            if len(_SENT_SESSIONS) >= _MAX_SENT_SESSIONS:
                _SENT_SESSIONS.clear()
            _SENT_SESSIONS[sid] = 1
        envelope = _session_envelope(sid, boundary, **kwargs)
        if envelope is None:
            return False
        SubstrateClient.from_env().post_json(
            "/api/v1/ledger/events",
...
def describe_surface() -> dict[str, Any]:
    """Inspectable map of Cowork surfaces to runtime entry points (no I/O)."""
    return {
        "toolset": TOOLSET,
        "hooks": dict(COWORK_SURFACE["hooks"]),
        "mcp_tools": list(COWORK_SURFACE["mcp_tools"]),
        "static_prompt_chars": len(STATIC_MEMORY_PROMPT),
    }
